...
def get_sp_tp(tree, beg, end, transdic):
	sp = ''
	tp = {}
...
def make_sen_input(tree, transdic, max_phrase_size, kbest, islog):
	...
	for i in range(len(tree)):
		...
		for j in range(i,k):
			...
			sp_tp = get_sp_tp(tree, tnode.beg , tnode.end ,transdic)
			if not sp_tp:
				continue
			tnode.s = sp_tp[0]
			tnode.t = sp_tp[1]
...
def print_reference(path, r_tree):
	...
	for tree in r_tree:
		for node in tree:
			...
			if node.dep != ROOT:
				depl = 'N'
			out.write(node.word + '\t' + node.tag + '\t' + node.depid + depl + '\n')
		out.write('\n')
	out.close()
...
if __name__=='__main__':
	import sys
	chtrees = read_dep_tree(sys.argv[1]) #ch tree
